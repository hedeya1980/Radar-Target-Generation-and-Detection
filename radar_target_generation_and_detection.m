...
num_tCells=(2*Tr+2*Gr+1)*(2*Td+2*Gd+1)-(2*Gr+1)*(2*Gd+1);
%making sure the loops are training and guard cells away from the edges of
%the 2d matrix
for i=Tr+Gr+1:m-(Gr+Tr)
    for j=Td+Gd+1:n-(Gd+Td)
        for p=i-(Tr+Gr):i+Tr+Gr
            for q=j-(Td+Gd):j+Td+Gd
                if(abs(i-p)>Gr || abs(j-q)>Gd)%exclude guard cells and CUT
        ...
        threshold_cfar(i,j)=pow2db(noise_level/num_tCells)+offset;
